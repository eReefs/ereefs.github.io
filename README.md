...
e.g. a top-level page `./docs/<filename>.md` will be compiled to  `/_site/<filename>.html`, and will be available at `/filename` in the published website. (And also at `/filename.html`, but the extension is not needed in URLs, and it's cleaner to omit it)
...
e.g. a file at `./docs/<foldername>/<childname>.md` will end up compiled to `/_site/<foldername>/<childname>.html`, and will be available at `/<foldername>/<childname>` in the published website.
...
- `redirect_from`: An array of additional relative URLs that will be redirected *to* this page (must not include the `permalink` or default page URL)
...
- [outputs](./docs/_outputs): one snippet for each eReefs output that should be included on the outputs pages
