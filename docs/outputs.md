---
permalink: /outputs/
redirect_from:
  - /tools/
  - /tools.html
  - /outputs.html
toc: true
tick_sticky: true
title: eReefs Outputs
---
{% assign active_outputs = site.outputs | where_exp: "item", "item.status != 'decommissioned'" %}

# Web Applications

The *eReefs* collaboration has developed several interactive web application to help you discover, explore and understand *eReefs* datasets:

<div class="tilegroup">
{% assign website_outputs = active_outputs | where_exp: "item", "item.category == 'website'" %}
{% for output in website_outputs %}
<div class="tile {{output.category}}" markdown="0">
  <a href="{{output.target_url}}" target="_window" title="Navigate to {{output.title}}">
    <i class="fas fa-{{output.fa-icon}}"></i>
    <h2>{{output.title}}</h2>
    {{output.caption | markdownify}}
    <img alt="Preview of {{output.title}}" src="{{output.preview_image}}" />
  </a>
</div>
{% endfor %}
</div>

[Discover more about *eReefs* web applications here](/outputs/web-applications)

---

# Open Access Datasets

The datasets generated by *eReefs* observing and modelling efforts are published with a [Creative Commons by Attribution (CC-BY 4.0)](https://creativecommons.org/licenses/by/4.0/legalcode) licence and may be freely re-used in other data products.

{% assign featured_datasets = active_outputs | where_exp: "item", "item.category == 'datasets'" | where_exp: "item", "item.feature" %}
{% if featured_datasets.size > 0 %}
<div class="tilegroup">
{% for output in featured_datasets %}
<div class="tile {{output.agency | slugify}} {{output.category | slugify}}" markdown="0">
  <a href="/outputs/open-access-datasets#{{output.title | slugify}}" target="_window" title="See details of {{output.title}}">
    <i class="fas fa-{{output.fa-icon}}"></i>
    <h2>{{output.title}}</h2>
    {{output.caption | markdownify}}
    {% if output.preview_image %}<img alt="Preview of {{output.title}}" src="{{output.preview_image}}" />{% endif %}
  </a>
</div>
{% endfor %}
</div>
{% endif %}

[See the full details about all *eReefs* datasets here](/outputs/open-access-datasets)

---

# Data Access Tools and Tutorials

These tools have been developed to help you discover and re-use *eReefs* data products:

<div class="tilegroup">
{% assign data_access_outputs = active_outputs | where_exp: "item", "item.category == 'data-access'" %}
{% for output in data_access_outputs %}
<div class="tile {{output.agency | slugify}} {{output.category | slugify}}" markdown="0">
  <a href="{{output.target_url}}" target="_window" title="Navigate to {{output.title}}">
    <i class="fas fa-{{output.fa-icon}}"></i>
    <h2>{{output.title}}</h2>
    {{output.caption | markdownify}}
    {% if output.preview_image %}<img alt="Preview of {{output.title}}" src="{{output.preview_image}}" />{% endif %}
  </a>
</div>
{% endfor %}
{% assign tutorials = active_outputs | where_exp: "item", "item.category == 'tutorial'" %}
{% for output in tutorials %}
<div class="tile {{output.agency | slugify}} {{output.category | slugify}}" markdown="0">
  <a href="{{output.target_url}}" target="_window" title="Navigate to {{output.title}}">
    <i class="fas fa-{{output.fa-icon}}"></i>
    <h2>{{output.title}}</h2>
    {{output.caption | markdownify}}
    {% if output.preview_image %}<img alt="Preview of {{output.title}}" src="{{output.preview_image}}" />{% endif %}
  </a>
</div>
{% endfor %}
</div>

[See more *eReefs* data access tools and tutorials here](/outputs/data-access-tools-tutorials)

---

# Open Source Software

Wherever possible, the software created to support, deliver and explore *eReefs* datasets has been released under an open-source licence so that it can be re-used and re-mixed by any other research teams who need it.

{% assign featured_software = active_outputs | where_exp: "item", "item.category == 'software'" | where_exp: "item", "item.feature" %}
{% if featured_software.size > 0 %}
Our featured *eReefs* software includes:
<div class="tilegroup">
{% for output in featured_software %}
<div class="tile {{output.agency | slugify}} {{output.category | slugify}}" markdown="0">
  <a href="/outputs/open-source-software#{{output.title | slugify}}" target="_window" title="Information about {{output.title}}">
    <i class="fas fa-{{output.fa-icon}}"></i>
    <h2>{{output.title}}</h2>
    {{output.caption | markdownify}}
    {% if output.preview_image %}<img alt="Preview of {{output.title}}" src="{{output.preview_image}}" />{% endif %}
  </a>
</div>
{% endfor %}
</div>
&nbsp;
{% endif %}

[Discover all *eReefs* open-source software here](/outputs/open-source-software)

---

# Scientific Publications

The *eReefs* research collaboration has resulted in a large number of peer-reviewed scientific publications.

[See the full list of *eReefs* publications here!](/outputs/scientific-publications)

---

# Impact Evaluations

How do we know if *eReefs* is worth the effort and funding invested in it over the years?  [Read independent evaluations of the impact of *eReefs* and related research programs here](/outputs/impact-evaluations).
